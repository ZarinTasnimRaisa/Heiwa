...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $reader_nid = $_POST["reader_nid"];
    $reader_name = $_POST["reader_name"];
    $reader_email = $_POST["reader_email"];
    $reader_password = $_POST["reader_password"];
    $confirmPassword = $_POST["confirmPassword"];

    if (strlen($reader_password) < 8 || strlen($reader_password) > 32) {
        $error_message = "Error: Password must be between 8 and 32 characters in length.";
        ...
        $nid_query = "SELECT * FROM reader WHERE reader_nid = '$reader_nid'";
...
.register p a:hover {
    ...
    <body>
<section>
    <form method="post" action="reader_signup.php">
        <h1>Reader Sign Up</h1>
        ...
        <div class="inputbox">
            <ion-icon name="mail-outline"></ion-icon>
            <input type="text" name="reader_nid" required>
            <label for="">NID</label>
        </div>
        ...
        <div class="register">
            <p>Already have an Account? <a href="http://localhost/HEIWA/reader_login.php">Log In</a></p>
        </div>
    </form>
</section>
</body>
